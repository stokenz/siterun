...
export const prefetchUseSitesServiceSearchSites = (queryClient: QueryClient, { code }: {
  code: unknown;
}) => queryClient.prefetchQuery({ queryKey: Common.UseSitesServiceSearchSitesKeyFn({ code }), queryFn: () => SitesService.searchSites({ code }) });
...
export const prefetchUseAssetsServiceFindManyAssets = (queryClient: QueryClient, { page, recordsPerPage }: {
  page: number;
  recordsPerPage: number;
}) => queryClient.prefetchQuery({ queryKey: Common.UseAssetsServiceFindManyAssetsKeyFn({ page, recordsPerPage }), queryFn: () => AssetsService.findManyAssets({ page, recordsPerPage }) });
export const prefetchUseAssetsServiceSearchAssets = (queryClient: QueryClient, { name }: {
  name: unknown;
}) => queryClient.prefetchQuery({ queryKey: Common.UseAssetsServiceSearchAssetsKeyFn({ name }), queryFn: () => AssetsService.searchAssets({ name }) });
export const prefetchUseAssetsServiceFindOneAsset = (queryClient: QueryClient, { id }: {
  id: string;
}) => queryClient.prefetchQuery({ queryKey: Common.UseAssetsServiceFindOneAssetKeyFn({ id }), queryFn: () => AssetsService.findOneAsset({ id }) });
